...
import static org.qubership.atp.environments.utils.PactTestUtils.formSystem;
...
@PactUrl(urls = {"file:./src/test/resources/pacts/atp-tdm-atp-environments.json"})
@AutoConfigureMockMvc(addFilters = false)
@WebMvcTest(controllers = {ProjectController.class, EnvironmentController.class, SystemController.class})
...
@Import({JacksonAutoConfiguration.class, HttpMessageConvertersAutoConfiguration.class,
        ...
        ProjectController.class, EnvironmentController.class, SystemController.class})
@Slf4j
@Isolated
public class EnvironmentsAndTdmContractTest {
    ...
    public void beforeAll() throws Exception {
        log.info("EnvironmentsAndTdmContractTest tests started");
        ...
        List<System> systems1 = formSystems();
        when(environmentService.getSystemsV2(any(UUID.class), any(String.class))).thenReturn(systems1);

        List<Project> projects = List.of(formProject());
        ...
        List<System> systems2 = formSystems();
        when(projectService.getSystemsByProjectId(any(UUID.class))).thenReturn(systems2);

        System system = formSystem();
        when(systemService.get(any(UUID.class))).thenReturn(system);

        List<System> systems3 = formSystems();
        when(projectService.getSystemsByProjectId(any(UUID.class))).thenReturn(systems3);
