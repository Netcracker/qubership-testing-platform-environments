...
public class EnvironmentCategoryControllerTest {
    ...
    List<EnvironmentCategory> categories = Arrays
            .asList(new EnvironmentCategoryImpl(UUID.randomUUID(), "Environment", "", "", 0L, 0L),
                    new EnvironmentCategoryImpl(UUID.randomUUID(), "TA Tool", "", "", 0L, 0L));


    @Test
    public void onEnvironmentCategoryController_GetAll_ListEnvironmentCategory() throws Exception {
        Mockito.when(environmentCategoryService.getAll()).thenReturn(categories);
        this.mockMvc.perform(MockMvcRequestBuilders.
                get("/api/environment-categories")
                .accept(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$[0]").exists())
                .andExpect(MockMvcResultMatchers.jsonPath("$[0].name").value("Environment"));
    }
    ...
    public void onEnvironmentCategoryController_GetById_EnvironmentCategory() throws Exception {
        ...
        this.mockMvc.perform(MockMvcRequestBuilders.
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("$.name").value("Environment"));
    }
