...
import static org.qubership.atp.environments.utils.PactTestUtils.formEnvironmentName;
...
@PactUrl(urls = {"file:./src/test/resources/pacts/atp-ram-atp-environments.json"})
...
@Slf4j
@Isolated
public class EnvironmentsAndRamContractTest {
    ...
    public void beforeAll() throws Exception {
        log.info("EnvironmentsAndRamContractTest tests started");

        List<Environment> environments = List.of(formEnvironment());
        when(environmentService.findBySearchRequest(any())).thenReturn(environments);

        Environment result = formEnvironment();
        when(environmentService.get(any(UUID.class))).thenReturn(result);

        String name = formEnvironmentName();
        when(environmentService.getEnvironmentNameById(any(UUID.class))).thenReturn(name);
    }
